...
class Path {
	protected:
		...
		int Path_ressource;							//Resource of the path
		std::vector<int> Path_vertexList;			//List of Vertex that conform the path.

	public:

		Path();
		Path(const std::vector<int>& l, double w, double r);
		~Path(void){}
		...
		/**
		 * @return total Resource consumed using the path.  
		 */
		int getRessource() const;

		/**
		 * @param total Resource consumed using the path.  
		 */
		void setRessource(int val);
		...
		bool subPath(std::vector<int>& sub_path, int endingVertex);
 		 ...
 		 */
		void addVertex(int v, double w, double c);
		...
		/**
		 * @return list of vertex that conform the path.
		 */
