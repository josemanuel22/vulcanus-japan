...
Path* Dijkstra::makePath(int source, int target) {
    //After all the calculation we just wrapper the information.(we make a path.)
	Path* path = new Path();
    if(previous[target]==UNDEFINED) {
        return NULL;
    }
	for(int p = target ; p!= source; p=previous[p]) {
		path->addVertex(p, 0, 0);
	}
	path->addVertex(source,0,0);
	path->setWeight(dis[target]);
    path = path->reverse(); //The path we obtain following the previous node from the target to the source is just the other way around. So we reversed.
    std::vector<int> r = path->getVertexList();
    path->setRessource(graph->getRessouce(r));
	return path;
}
