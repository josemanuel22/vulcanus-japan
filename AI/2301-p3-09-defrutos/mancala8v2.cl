...
(defun minimax (estado profundidad-max f-eval)
  (let* ((oldverb *verb*) ; Si la bandera de debug esta puesta modo verboso en la ejecucion de minimax. Si no modo vervoso desactivado
         ...
         (estado2 (minimax-1 estado 0 t profundidad-max f-eval)) ; Llamada a minimax desde la profundidad inicial 0 (modo verboso o no). Es decir empezamos la busqueda como tal.
         (*verb* oldverb)) ;Restabecemos el valor de *verb*.
    estado2))


;;; minimax-1
;;; Funcion que inicia la busqueda desde un estado dado a un profundidad establecida.
;;; Devuelve el valor del tablero optimo segun minimax o la jugada (sucesor) que conduce a dicho estado. (depende de evolver-movimiento)
;;; tiene el turno, segun algoritmo minimax
;;; RECIBE:   estado,
;;;           profundidad-max    : límite de profundidad
;;;           f-eval             : función de evaluación
;;;           devolver-movimiento : flag que indica si se devuelve el estado siguiente de la partida segun el algoritmo minimax,
;;;                                 o el valor del estado siguiente de la partida segun el algoritmo MINIMAX.
;;;                                 Si es estado raiz -> siguiente estado de la partida. devolver-movimiento=T
;;;                                 si no devolver la evaluacion del estado que maximiza. devolver-movimiento=nil
;;; DEVUELVE: estado siguiente del juego
;;; ------------------------------------------------------------------------------------------
...
(defun minimax-1 (estado profundidad devolver-movimiento profundidad-max f-eval)
  (cond ((>= profundidad profundidad-max) ;En caso de llegar a la profundidad maxima estabelcida. Paramos la busqueda y evaluamos el tablero.
        ...
        ;;En caso contrario la evalucion del estado.
        ...
        (t
         (let* ((sucesores (generar-sucesores estado)) ;Generamos todos los tableros sucesores y realizamos la busqueda
               (mejor-valor -99999) ;El valor de un estado es inicialmente para max -infinto y para min +inf
           ...
           (cond ((null sucesores) ;Si ya no quedan sucecesores en la lista hemos llegado al final de nuestra exploracion y evaluamos el estado.
                 ...
                 (t
                  ...
                  (if  devolver-movimiento mejor-sucesor mejor-valor))))))) ;Devolvemos el valor optimo o el sucesor que concude a ducho tablero con valor optimo.
...
;;; ------------------------------------------------------------------------------------------
(defun f-eval-pepeelromano (estado)
  (+ (* 2 (- (get-tot (estado-lado-sgte-jugador estado))
    (get-tot (lado-contrario (estado-lado-sgte-jugador estado)))))

    (* 4 (- (get-pts (estado-lado-sgte-jugador estado))
    (get-pts (lado-contrario (estado-lado-sgte-jugador estado)))))

    (* 4 (- (suma-fila (estado-tablero estado) (estado-lado-sgte-jugador estado) 5)
      (suma-fila (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 5)))))


;;; f-juego primero
;;; ------------------------------------------------------------------------------------------
(setf *jdr-mmx-pepeelromano* (make-jugador
                        :nombre   '|Ju-Mmx-PepeElRomano|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-pepeelromano))


(defun f-eval-ron (estado)
...
(defun f-eval-neville (estado)
 (+
 (- (length (posiciones-con-fichas-lado (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 1))
  (length (posiciones-con-fichas-lado (estado-tablero estado) (estado-lado-sgte-jugador estado) 1)))
 (if (estado-debe-pasar-turno estado) 5 -5)))



;;--------------------------------------------------------------------------------------------
(defun mi-f-jonymelavo (estado)
  (* ( - (get-pts (estado-lado-sgte-jugador estado)) ; Diferencia de fichas. + si vas ganando - en caso contrario
    (get-pts (lado-contrario (estado-lado-sgte-jugador estado))))

    (- 36 (min (suma-fila (estado-tablero estado) (estado-lado-sgte-jugador estado) 5) ;Apostamos por partidas cortas. Es decir si la partida le falta poco para finalizar mejor.
    (suma-fila (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 5)))))


;;--------------------------------------------------------------------------------------------
(defun mi-f-heynigga (estado)
  (if (>= (get-pts (estado-lado-sgte-jugador estado)) 18)
             (* (get-pts (estado-lado-sgte-jugador estado)) (- 36 (min (suma-fila (estado-tablero estado) (estado-lado-sgte-jugador estado) 5)
                (suma-fila (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 5))))
       (*(get-pts (estado-lado-sgte-jugador estado)) (min (suma-fila (estado-tablero estado) (estado-lado-sgte-jugador estado) 5)
    (suma-fila (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 5)))))


;;; f-juego primero
;;; ------------------------------------------------------------------------------------------
(setf *jdr-mmx-neville* (make-jugador
                        :nombre   '|Ju-Mmx-neville|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-neville))
...
(defun f-eval-malfoy (estado)
  (+ (* 5 (aux1-malfoy estado)) (* 2 (aux2-malfoy estado)) (aux3-malfoy estado)))


(setf *jdr-mmx-malfoy* (make-jugador
                        :nombre   '|Ju-Mmx-malfoy|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-malfoy))



(defun aux1-harry (estado)
  (- (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 6)
      (get-fichas (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 6)))

(defun aux2-harry (estado)
  (if (juego-terminado-p estado)
    (cond ((> (get-pts (estado-lado-sgte-jugador estado)) 18) 9999)
          (( =  (get-pts (estado-lado-sgte-jugador estado)) 18) 0)
          ( t (- 9999)))
    0))

(defun aux3-harry (estado)
  (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 5))

(defun aux4-harry (estado)
  (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 4))

(defun aux5-harry (estado)
  (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 0))

(defun f-eval-harry (estado)
  (+ (* 5 (aux1-harry estado)) (* 1 (aux2-harry estado)) (* 2 (aux3-harry estado)) (* 1 (aux4-harry estado))))


(setf *jdr-mmx-harry* (make-jugador
                        :nombre   '|Ju-Mmx-harry|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-harry))


(defun f-eval-hermione (estado)
...
;(defun aux3-hermione (estado)
;  (+ (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 2)
;    (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 3)))

(setf *jdr-mmx-hermione* (make-jugador
                        :nombre   '|Ju-Mmx-hermione|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-hermione))



(defun aux1-voldemor (estado)
  (- (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 6)
      (get-fichas (estado-tablero estado) (lado-contrario (estado-lado-sgte-jugador estado)) 6)))

(defun aux2-voldemor (estado)
  (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 5))

(defun aux3-voldemor (estado)
  (get-fichas (estado-tablero estado) (estado-lado-sgte-jugador estado) 0))

(defun f-eval-voldemor (estado)
  (+ (* *njugada* (aux1-voldemor estado)) (* 3 (aux2-voldemor estado)) (* 2 (aux3-voldemor estado))))


(setf *jdr-mmx-voldemor* (make-jugador
                        :nombre   '|Ju-Mmx-voldemor|
                        :f-juego  #'f-j-mmx
                        :f-eval   #'f-eval-voldemor))
...
;;; ajustar-peso
;;;
;;; Funcion que ajusta los pesos de ciertas medidas heuristicas y crea un jugador cuya heuristica esta
...
;;;
;;; Objetivo:
;;;  Buscar el maximo dado por la combinacion de heuristicas básicas.
;;;
...
;;;      (+ (* i (aux1-malfoy estado)) (* j (aux2-malfoy estado))))
;;;
...
;;;    (do ((j start (+ j 1)))
;;;      ((> (+ i j) 100))
;;;        (when (and (= (partida 0 2 (list jugador     *jdr-mmx-p*)) 2) (= (partida 1 2 (list *jdr-mmx-malfoy*     *jdr-mmx-p*)) 2))
;;;          (format str "~D ~D" i j))))
;;;    (close str))



;;; (ajustar-peso *jdr-mmx-malfoy*)
...
;(partida 0 2 (list *jdr-mmx-malfoy*      *jdr-mmx-harry*))
;(partida 0 3 (list *jdr-mmx-malfoy*      *jdr-mmx-harry*))
;(partida 0 2 (list *jdr-mmx-regular*      *jdr-mmx-harry*))
;(partida 0 2 (list *jdr-mmx-regular*     *jdr-mmx-malfoy*))
;(partida 0 2 (list *jdr-mmx-malfoy*     *jdr-mmx-hermione*))
...
;;Apdo B2.1 (a)
(setq mi-posicion (list '(1 0 1 1 0 0 7) (reverse '(7 0 0 0 1 1 1))))
(setq estado (crea-estado-inicial 0 mi-posicion))
(generar-sucesores estado)

(minimax-1 estado 0 t 1 #'f-eval-snape)
(minimax-1 estado 0 nil 1 #'f-eval-snape)

(minimax-1 estado 0 t 2 #'f-eval-snape)
(minimax-1 estado 0 nil 2 #'f-eval-snape)

(setq mi-posicion (list '(1 0 0 1 0 0 7) (reverse '(7 0 0 0 1 0 1))))
(setq estado (crea-estado-inicial 0 mi-posicion))
(generar-sucesores estado)
(minimax-1 estado 0 t 3 #'f-eval-snape)
(minimax-1 estado 0 nil 3 #'f-eval-snape)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Funciones alfa-beta que exploran los sucucesore en ordenes diferentes al habitual.
...
;;; jugador que usa minimax con poda.
(defun f-j-mmx-a-b (estado profundidad-max f-eval)
   (minimax-a-b estado profundidad-max f-eval))

;;; jugador que usa minimax con poda y ordenacion de los nodos de mayor a menor valor heuristico
(defun f-j-mmx-a-b-sort (estado profundidad-max f-eval)
   (minimax-a-b-sort estado profundidad-max f-eval))

;;; jugador que usa minimax con poda y ordenacion de los nodos de menor a mayor valor heuristico
(defun f-j-mmx-a-b-unsort (estado profundidad-max f-eval)
   (minimax-a-b-unsort estado profundidad-max f-eval))
...
;;; Pareja de jugadores con la misma heuristica y que minimax con poda y desordenacion de nodos.
(setf *mi-jugador-pruebas-a-b-1-unsort* (make-jugador
    :nombre 'Jugador-Minimax-1-unsort
    :f-juego #'f-j-mmx-a-b-unsort
    :f-eval #'f-eval-snape))

(setf *mi-jugador-pruebas-a-b-2-unsort* (make-jugador
    :nombre 'Jugador-Minimax-2-unsort
    :f-juego #'f-j-mmx-a-b-unsort
    :f-eval #'f-eval-snape))


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Misma partida, jugadores con la misma heuristica con/sin poda  y con/sin ordenacion para comparar tiempos
;;;

;;(time (partida 0 3 (list *mi-jugador-pruebas-1* *mi-jugador-pruebas-2*))) ; 1. Partida entre jugadores que usan minimax sin poda.
;;(time (partida 0 3 (list *mi-jugador-pruebas-a-b-1* *mi-jugador-pruebas-a-b-2*))) ; 2. Partida entre jugadores que usan minimax con poda.
;;(time (partida 0 3 (list *mi-jugador-pruebas-a-b-1-sort* *mi-jugador-pruebas-a-b-2-sort*)));3. Partida entre jugadores que usan minimax con poda y ordenacion
;;(time (partida 0 3 (list *mi-jugador-pruebas-a-b-1-unsort* *mi-jugador-pruebas-a-b-2-unsort*))) ;4. Partida entre jugadores que usan minimax con poda y ordenacion inversa

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Conclusion:
;;; La partida 2. tarda en general menos (de manera apreciable) que la partida 1. La poda ahorra la busqueda de ciertas ramas y esto conduce a un ahoro de tiempo.
;;; En general podemos considerar que los sucesores generados estan ordenados de manera aleatoria.
;;;
;;; Sin embargo, la partida con poda y ordenacion de nodos (3.)  tarda aproximandamente igual que 2. Teoricamente debería tardar menos pero hay que añadir el sobre coste
;;; de ordenar. A profundidades pequenas no se nota la ventaja de explorar nodos ordenados.
;;;
;;; (4.) Con odenacion inversa (se ordena los nodos de menor a mayor valor heuristico) los resultados son los peores logicamente. Tarda para profundidad pequena mas que sin poda.
;;; Ya que al explorar los nodos optimos al final no se poda nunca luego tarda lo mismo que el algoritmo sin poda pero además hay que anadir el sobre coste de ordenar.

;;(time (partida 0 5 (list *mi-jugador-pruebas-1* *mi-jugador-pruebas-2*)))
;;(time (partida 0 5 (list *mi-jugador-pruebas-a-b-1* *mi-jugador-pruebas-a-b-2*)))
;;(time (partida 0 5 (list *mi-jugador-pruebas-a-b-1-sort* *mi-jugador-pruebas-a-b-2-sort*)))
;;(time (partida 0 5 (list *mi-jugador-pruebas-a-b-1-unsort* *mi-jugador-pruebas-a-b-2-unsort*)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Conclusion:
;;;
;;; En este caso ya si se nota una diferencia entre 2. y 3.. Ya que la la busqueda crece de manera exponencial y
;;; la ordenacion de manera n*log n. Luego para valores relativamentes grandes de profundidad conviene ordenar los sucesores
;;; por valor heuristico (de mayor a menor).
;;;
;;; La ordenacion inversa da resultados parecidos al algoritmo sin poda ya que el sobre coste de ordenar se atenua debido al caracter exponecial de la busqueda.
