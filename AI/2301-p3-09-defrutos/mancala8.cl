...
;;;
;;; MANCALA - v.8.1 P2P   A. de Salabert 2014
...
;;; - Converge mas rapido por lo que se reduce a 40 el numero maximo de jugadas
;;;
...
(defvar *vers*           "8.1" ) ; Software version "x.y" = mancala X, rel. Y
...
(defvar *maxjugadas*       40  ) ; num maximo de jugadas antes de dar la partida por tablas
         ...
         (T                                                     ; llamada al jugador que tiene el turno
          (cond
           ((estado-debe-pasar-turno estado)                    ; TBD No registra accion!
            (format t "~%[J ~A] ~A pasa turno." *njugada* (jugador-nombre curr-plyr))
...
(defun minimax (estado profundidad-max f-eval)
  (let* ((oldverb *verb*) ;Conjunto de abreviaciones.
         (*verb* (if *debug-mmx* *verb* nil))
         (estado2 (minimax-1 estado 0 t profundidad-max f-eval)) ; Llamada a minimax desde la profundidad inicial 0. Es decir empezamos la busqueda como tal.
         (*verb* oldverb))
    estado2)) 
                 ...
                 (t
                  (loop for sucesor in sucesores do
                    (let* ((resultado-sucesor (minimax-1 sucesor (1+ profundidad)
                      ...
                      when (beta >= alfa) ;Te prometo que la sintaxis de los loops es esta
                        do (return)
                      (when (> valor-nuevo mejor-valor)
                        (setq mejor-valor valor-nuevo)
                        (setq mejor-sucesor  sucesor ))))
                  (if  devolver-movimiento mejor-sucesor mejor-valor))))))); La bandera devolver-movimiento indica si hay que devolver el movimiento que segun la busqueda es la mejor opcion o hay que el valor asociado al estado.
                                                                          ; Para nodos no raiz habra que devolver el valor asodiado al estado. para el nodo raiz el moviento a relizar.
...
;;; ------------------------------------------------------------------------------------------
 ...
 (defun minimax-a-b (estado profundidad-max f-eval)
  (minimax-1-a-b estado 0 profundidad-max -99999 99999 f-eval))
 ...
 (defun minimax-1-a-b (estado profundidad devolver-movimiento profundidad-max alfa beta f-eval)
    (if (>= profundidad profundidad-max)
      ...
      (if (null sucesores)
        ...
        (if (>= alfa beta)
        (loop for sucesor in sucesores do
          (let* ((resultado-sucesor (minimax-1-a-b sucesor (1+ profundidad)
...
(partida 0 2 (list *jdr-mmx-neville*      *jdr-mmx-Ron*))
